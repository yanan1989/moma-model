...
from torchvision.transforms.functional import center_crop, crop, get_dimensions
...
class RandomCropWithBoxes(nn.Module):
  def __init__(self, size):
    ...
    self._size = tuple(_setup_size(size, error_msg='Please provide only two dimensions (h, w) for size.'))

  def forward(self, images, boxes):
    _, height, width = get_dimensions(images)
    i, j, h, w = self.get_params(images, self._size)

    images = crop(images, i, j, h, w)
    boxes = crop_boxes(boxes, j, i)
    boxes = clip_boxes_to_image(boxes, self.size[0], self.size[1])
...
class RandomResizedCropWithBoxes(nn.Module):
  ...
  def forward(self, images, boxes):
    ...
    boxes = crop_boxes(boxes, j, i)
    boxes[:, [0, 2]] *= float(self._target_height)/h
    boxes[:, [1, 3]] *= float(self._target_width)/w
...
class CenterCropWithBoxes(nn.Module):
  ...
  def forward(self, images, boxes):
    ...
    y_offset, x_offset = int(math.ceil((height-self._size)/2))

    images = center_crop(images, self._size)
    boxes = crop_boxes(boxes, x_offset, y_offset)
    boxes = clip_boxes_to_image(boxes, boxes.shape[-2], boxes.shape[-1])
    return images, boxes
