# Reference: https://raw.githubusercontent.com/antoine77340/S3D_HowTo100M/master/s3dg.py
...
class S3D(nn.Module):
  def __init__(self, dict_path, head=None, output_dim=512, gating=True, space_to_depth=True):
    ...
    self.fc = nn.Linear(self.mixed_5c.output_dim, output_dim)
    self.text_module = Sentence_Embedding(output_dim, token_to_word_path=dict_path)
...
def create_head(out_feature):
  return create_res_basic_head(in_features=1024,
...
def create_multi_head(out_features):
  heads = []
  for out_feature in out_features:
    heads.append(create_head(out_feature))
